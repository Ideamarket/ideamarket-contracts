...
describe("timelock/DSPauseProxy", () => {
  ...
  beforeEach(async () => {
    ...
    dsPauseProxy = new ethers.Contract(
      await dsPause._proxy(),
      DSPauseProxy.interface,
      DSPauseProxy.signer
    )
  ...
  it("fail unauthorized exec", async () => {
    await expect(dsPauseProxy.exec(zeroAddress, [])).to.be.revertedWith(
      "ds-pause-proxy-unauthorized"
    )
  })
  ...
  it("fail delegatecall error", async () => {
    ...
    await expect(dsPause.exec(spell.address, tag, fax, eta)).to.be.revertedWith(
