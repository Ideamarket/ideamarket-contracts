...
describe("timelock/DSPause", () => {
  ...
  it("admin and user cannot set owner", async () => {
    await expect(
      dsPause.connect(adminAccount).setOwner(someAddress)
    ).to.be.revertedWith("ds-pause-undelayed-call")
    await expect(
      dsPause.connect(userAccount).setOwner(someAddress)
    ).to.be.revertedWith("ds-pause-undelayed-call")
  })

  it("admin and user cannot set delay", async () => {
    await expect(
      dsPause.connect(adminAccount).setDelay(BigNumber.from("0"))
    ).to.be.revertedWith("ds-pause-undelayed-call")
    await expect(
      dsPause.connect(userAccount).setDelay(BigNumber.from("0"))
    ).to.be.revertedWith("ds-pause-undelayed-call")
  })
  ...
  it("user cannot plot", async () => {
    ...
    const tag = await dsPause.soul(spell.address)
    await expect(
      dsPause.connect(userAccount).plot(spell.address, tag, [], eta)
    ).to.be.revertedWith("ds-pause-unauthorized")
  })
  ...
  it("user cannot drop", async () => {
    ...
    await dsPause.plot(spell.address, tag, [], eta)
    await expect(
      dsPause.connect(userAccount).drop(spell.address, tag, [], eta)
    ).to.be.revertedWith("ds-pause-unauthorized")
  })
  ...
  it("user cannot exec", async () => {
    ...
    await dsPause.plot(spell.address, tag, [], eta)
    await expect(
      dsPause.connect(userAccount).exec(spell.address, tag, [], eta)
    ).to.be.revertedWith("ds-pause-unauthorized")
  })
  ...
  it("cannot exec unplotted", async () => {
    ...
    await dsPause.plot(spell.address, tag, [], eta)
    await expect(
      dsPause.exec(spell.address, tag, [], eta.add(BigNumber.from("1")))
    ).to.be.revertedWith("ds-pause-unplotted-plan")
  })
  ...
  it("cannot exec premature", async () => {
    ...
    const tag = await dsPause.soul(spell.address)
    await dsPause.plot(spell.address, tag, [], eta)
    await expect(dsPause.exec(spell.address, tag, [], eta)).to.be.revertedWith(
  ...
  it("cannot disregard delay", async () => {
    ...
    const tag = await dsPause.soul(spell.address)
    await expect(dsPause.plot(spell.address, tag, [], eta)).to.be.revertedWith(
