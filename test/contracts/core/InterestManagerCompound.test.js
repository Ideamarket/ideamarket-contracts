...
describe("core/InterestManagerCompound", () => {
  ...
  it("admin is owner", async () => {
    expect(adminAccount.address).to.equal(
      await interestManagerCompound.getOwner()
    )
  })
  ...
  it("can invest", async () => {
    ...
    expect(tenPow18.eq(await cDai.balanceOf(interestManagerCompound.address)))
      .to.be.true
  })

  it("fail invest too few dai", async () => {
    await expect(interestManagerCompound.invest(tenPow18)).to.be.revertedWith(
  ...
  it("can redeem", async () => {
    ...
    await interestManagerCompound
      ...
      .redeem(adminAccount.address, redeemAmount)

    expect(redeemAmount.eq(await dai.balanceOf(adminAccount.address))).to.be
      .true
    expect(
      tenPow18
        .sub(redeemAmount)
        .eq(await cDai.balanceOf(interestManagerCompound.address))
    ).to.be.true
  })
  ...
  it("fail redeem not admin", async () => {
    ...
    const redeemAmount = tenPow18.div(BigNumber.from("2"))
    await expect(
      interestManagerCompound.redeem(adminAccount.address, redeemAmount)
  ...
  it("can donate interest and redeem donated dai", async () => {
    ...
    expect(zero.eq(await cDai.balanceOf(interestManagerCompound.address))).to.be
      .true
  })
  ...
  it("fail donate not enough allowance", async () => {
    await expect(
      interestManagerCompound.donateInterest(BigNumber.from("100"))
    ).to.be.revertedWith("donateInterest: not enough allowance")
  })
  ...
  it("cannot redeem more than donated", async () => {
    ...
    await expect(
      interestManagerCompound.redeemDonated(tenPow18.mul(BigNumber.from("2")))
    ).to.be.revertedWith("redeemDonated: not enough donated")
  })
  ...
  it("can withdraw COMP", async () => {
    ...
    expect(
      (await comp.balanceOf(interestManagerCompound.address)).eq(
        BigNumber.from("0")
      )
    ).to.be.true
    expect((await comp.balanceOf(compRecipient.address)).eq(compBalance)).to.be
      .true
  })
})
