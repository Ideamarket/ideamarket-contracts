...
describe("core/IdeaTokenExchange", () => {
  ...
  before(async () => {
    ...
    DomainNoSubdomainNameVerifier = await ethers.getContractFactory(
      "DomainNoSubdomainNameVerifier"
    )
    TestERC20 = await ethers.getContractFactory("TestERC20")
    TestCDai = await ethers.getContractFactory("TestCDai")
    InterestManagerCompound = await ethers.getContractFactory(
  ...
  beforeEach(async () => {
    ...
    await ideaTokenFactory
      .connect(adminAccount)
      .initialize(adminAccount.address, ideaTokenExchange.address)
    ...
    await ideaTokenExchange
      .connect(adminAccount)
      .setIdeaTokenFactoryAddress(ideaTokenFactory.address)
  ...
  it("can buy and sell 500 tokens with correct interest", async () => {
    ...
    expect((await dai.balanceOf(userAccount.address)).eq(BigNumber.from("0")))
      .to.be.true
    expect((await ideaToken.balanceOf(userAccount.address)).eq(amount)).to.be
      .true
    expect(
    ...
    expect(
      (await ideaTokenExchange.getInterestPayable(ideaToken.address)).eq(
        BigNumber.from("0")
      )
    ).to.be.true
    ...
    expect(
      (await ideaTokenExchange.getInterestPayable(ideaToken.address)).eq(
        firstInterestPayable
      )
    ).to.be.true

    const secondCost = await ideaTokenExchange.getCostForBuyingTokens(
      ideaToken.address,
      amount
    )
    const secondTradingFee = await getTradingFeeForBuying(ideaToken, amount)
    const secondTradingFeeInvested = secondTradingFee
      .mul(tenPow18)
      .div(firstExchangeRate)
    const secondPlatformFee = await getPlatformFeeForBuying(ideaToken, amount)
    const secondPlatformFeeInvested = secondPlatformFee
      .mul(tenPow18)
      .div(firstExchangeRate)
    const secondRawCost = secondCost
      .sub(secondTradingFee)
      .sub(secondPlatformFee)
    expect(secondCost.eq(await getCostForBuyingTokens(ideaToken, amount))).to.be
      .true
    ...
    expect((await dai.balanceOf(userAccount.address)).eq(BigNumber.from("0")))
      .to.be.true
    expect(
      (await ideaToken.balanceOf(userAccount.address)).eq(amount.add(amount))
    ).to.be.true
    expect(
      (await ideaTokenExchange.getTradingFeePayable()).eq(
    ...
    expect(
      (await ideaTokenExchange.getInterestPayable(ideaToken.address)).eq(
        secondInterestPayable
      )
    ).to.be.true

    const firstPrice = await ideaTokenExchange.getPriceForSellingTokens(
      ideaToken.address,
      amount
    )
    const thirdTradingFee = await getTradingFeeForSelling(ideaToken, amount)
    const thirdTradingFeeInvested = thirdTradingFee
      .mul(tenPow18)
      .div(secondExchangeRate)
    const thirdPlatformFee = await getPlatformFeeForSelling(ideaToken, amount)
    const thirdPlatformFeeInvested = thirdPlatformFee
      .mul(tenPow18)
      .div(secondExchangeRate)
    ...
    await ideaTokenExchange.sellTokens(
      ideaToken.address,
      amount,
      firstPrice,
      userAccount.address
    ...
    const secondPrice = await ideaTokenExchange.getPriceForSellingTokens(
      ideaToken.address,
      amount
    )
    const fourthTradingFee = await getTradingFeeForSelling(ideaToken, amount)
    const fourthTradingFeeInvested = fourthTradingFee
      .mul(tenPow18)
      .div(thirdExchangeRate)
    const fourthPlatformFee = await getPlatformFeeForSelling(ideaToken, amount)
    const fourthPlatformFeeInvested = fourthPlatformFee
      .mul(tenPow18)
      .div(thirdExchangeRate)
    const secondRawPrice = secondPrice
      .add(fourthTradingFee)
      .add(fourthPlatformFee)
    expect(secondPrice.eq(await getPriceForSellingTokens(ideaToken, amount))).to
      .be.true
    ...
    expect(
      (await dai.balanceOf(userAccount.address)).eq(firstPrice.add(secondPrice))
    ).to.be.true
    expect(
      (await ideaToken.balanceOf(userAccount.address)).eq(BigNumber.from("0"))
    ).to.be.true
    expect(
      (await ideaTokenExchange.getTradingFeePayable()).eq(
        firstTradingFeeInvested
    ...
    const fourthInterestPayable = firstRawCost
      ...
      .div(tenPow18)
      .sub(
        firstRawCost.add(secondRawCost).sub(firstRawPrice).sub(secondRawPrice)
      )
    ...
    expect((await ideaTokenExchange.getTradingFeePayable()).eq(finalTradingFee))
      .to.be.true

    await ideaTokenExchange
      .connect(adminAccount)
      .authorizePlatformFeeWithdrawer(
        marketID,
        platformFeeReceiverAccount.address
      )

    await ideaTokenExchange
      .connect(platformFeeReceiverAccount)
      .withdrawPlatformFee(marketID)
    expect(
      (await dai.balanceOf(platformFeeReceiverAccount.address)).eq(
        finalPlatformFee
      )
    ).to.be.true
    ...
    await ideaTokenExchange
      .connect(adminAccount)
      .authorizeInterestWithdrawer(
        ideaToken.address,
        interestReceiverAccount.address
      )

    await ideaTokenExchange
      .connect(interestReceiverAccount)
      .withdrawInterest(ideaToken.address)
    // TODO: Minor rounding error
    // expect((await dai.balanceOf(interestReceiverAccount)).eq(fourthInterestPayable))

    await ideaTokenExchange.withdrawTradingFee()
    expect((await dai.balanceOf(tradingFeeAccount.address)).eq(finalTradingFee))
      .to.be.true
    expect(
      (await ideaTokenExchange.getTradingFeePayable()).eq(BigNumber.from("0"))
    ).to.be.true
  })
  ...
  it("can fallback on buy", async () => {
    ...
    const cost = await ideaTokenExchange.getCostForBuyingTokens(
      ideaToken.address,
      tenPow18
    )
  ...
  it("fail buy/sell - max cost / minPrice", async () => {
    ...
    const price = await ideaTokenExchange.getPriceForSellingTokens(
      ideaToken.address,
      tenPow18
    )
  ...
  it("no trading fee available", async () => {
    ...
    await ideaTokenExchange.withdrawTradingFee()
    expect(
      (await dai.balanceOf(tradingFeeAccount.address)).eq(BigNumber.from("0"))
    ).to.be.true
  })
  ...
  it("no interest available", async () => {
    ...
    await ideaTokenExchange
      ...
      .withdrawInterest(ideaToken.address)
    expect(
      (await dai.balanceOf(interestReceiverAccount.address)).eq(
        BigNumber.from("0")
      )
    ).to.be.true
  })
  ...
  it("fail authorize interest withdrawer not authorized", async () => {
    await expect(
      ideaTokenExchange.authorizeInterestWithdrawer(
        ideaToken.address,
        interestReceiverAccount.address
      )
  ...
  it("fail cannot set factory address twice", async () => {
    ...
    await expect(
      exchange.connect(adminAccount).setIdeaTokenFactoryAddress(someAddress)
    ).to.be.reverted
  })
  ...
  it("authorizer can set interest withdrawer", async () => {
    await ideaTokenExchange
      ...
      .authorizeInterestWithdrawer(ideaToken.address, someAddress)
  })
  ...
  it("authorizer can set platform fee withdrawer", async () => {
    await ideaTokenExchange
      .connect(authorizerAccount)
      .authorizePlatformFeeWithdrawer(marketID, someAddress)
  })
  ...
  function getRawCostForBuyingTokens(b, r, supply, amount) {
    ...
    const priceAtSupplyPlusAmount = b.add(
      r.mul(supply.add(amount)).div(tenPow18)
    )
    const average = priceAtSupply
      .add(priceAtSupplyPlusAmount)
  ...
  function getRawPriceForSellingTokens(b, r, supply, amount) {
    const priceAtSupply = b.add(r.mul(supply).div(tenPow18))
    const priceAtSupplyPlusAmount = b.add(
      r.mul(supply.sub(amount)).div(tenPow18)
    )
    const average = priceAtSupply
      .add(priceAtSupplyPlusAmount)
      .div(BigNumber.from("2"))
  ...
  async function getTradingFeeForBuying(token, amount) {
    ...
    const rawCost = getRawCostForBuyingTokens(
      baseCost,
      priceRise,
      supply,
      amount
  ...
  async function getTradingFeeForSelling(token, amount) {
    const supply = await token.totalSupply()
    const rawPrice = getRawPriceForSellingTokens(
      baseCost,
      priceRise,
      supply,
      amount
