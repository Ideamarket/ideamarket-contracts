...
describe('core/CurrencyConverter', () => {
	...
	beforeEach(async () => {
		...
		await router.connect(adminAccount).addLiquidity(weth.address,
			dai.address,
			ethAmount,
			daiAmount,
			ethAmount,
		...
		await router.connect(adminAccount).addLiquidity(someToken.address,
			dai.address,
			someAmount,
			daiAmount,
			someAmount,
	...
	it('fail buy cost too high', async () => {
		...
		expect(currencyConverter.buyTokens(someToken.address,
			ideaToken.address,
			ideaTokenAmount,
			ideaTokenAmount,
			requiredInputForCost.sub(BigNumber.from('1')),
			userAccount.address)).to.be.revertedWith('')
	})
	...
	it('fail sell price too low', async () => {
		...
		expect(
			currencyConverter.sellTokens(someToken.address,
				...
				userAccount.address)).to.be.revertedWith('sellTokens: slippage too high')
	})
