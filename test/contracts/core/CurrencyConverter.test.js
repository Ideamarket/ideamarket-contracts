...
describe("core/CurrencyConverter", () => {
  ...
  before(async () => {
    ...
    TestTransferHelperLib = await ethers.getContractFactory(
      "TestTransferHelper"
    )
    TestUniswapV2Lib = await ethers.getContractFactory("TestUniswapV2Library")
    TestUniswapV2Factory = await ethers.getContractFactory(
      "TestUniswapV2Factory"
    )
    TestUniswapV2Router02 = await ethers.getContractFactory(
  ...
  beforeEach(async () => {
    ...
    router = await TestUniswapV2Router02.deploy(
    ...
    await uniswapFactory
      .connect(adminAccount)
      .createPair(weth.address, dai.address)
    await router
      .connect(adminAccount)
      .addLiquidity(
        weth.address,
    ...
    await uniswapFactory
      .connect(adminAccount)
      .createPair(someToken.address, dai.address)
  ...
  it("can buy/sell tokens ETH", async () => {
    ...
    const buyCost = await ideaTokenExchange.getCostForBuyingTokens(
      ideaToken.address,
      ideaTokenAmount
    )
    const requiredInputForCost = (
      await router.getAmountsIn(buyCost, [weth.address, dai.address])
    )[0]
    ...
    const sellPrice = await ideaTokenExchange.getPriceForSellingTokens(
      ideaToken.address,
      tokenBalanceAfterBuy
    )
    const outputFromSell = (
      await router.getAmountsOut(sellPrice, [dai.address, weth.address])
    )[1]
  ...
  it("can buy/sell tokens SOME", async () => {
    ...
    const requiredInputForCost = (
      await router.getAmountsIn(buyCost, [someToken.address, dai.address])
    )[0]
    ...
    const outputFromSell = (
      await router.getAmountsOut(sellPrice, [dai.address, someToken.address])
    )[1]
