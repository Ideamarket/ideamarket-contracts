...
describe('core/IdeaTokenFactory', () => {
	...
	it('only admin can add market', async () => {
		await expect(
			ideaTokenFactory.connect(userAccount).addMarket(
				marketName, '0x0000000000000000000000000000000000000000',
				baseCost, priceRise,
				tradingFeeRate, platformFeeRate
	...
	it('fail add token with invalid name', async () => {
		...
		await expect(ideaTokenFactory.addToken('some.invalid.name', BigNumber.from('1'))).to.be.revertedWith('addToken: name verification failed')
	})
	...
	it('fail add token with same name twice', async () => {
		...
		await ideaTokenFactory.addToken(tokenName, BigNumber.from('1'))
		await expect(
			ideaTokenFactory.addToken(tokenName, BigNumber.from('1'))).to.be.revertedWith('addToken: name verification failed')
	})
	...
	it('fail add token invalid market', async () => {
		...
		await ideaTokenFactory.addToken(tokenName, BigNumber.from('1'))
		await expect(
			ideaTokenFactory.addToken(tokenName, BigNumber.from('2'))).to.be.revertedWith('addToken: market does not exist')
	})
	...
	it('fail user sets trading fee', async () => {
		...
		await expect(
			ideaTokenFactory.connect(userAccount).setTradingFee(BigNumber.from('1'), BigNumber.from('123'))).to.be.revertedWith('Ownable: onlyOwner')
	})
	...
	it('fail set trading fee invalid market', async () => {
		...
		await expect(
			ideaTokenFactory.connect(adminAccount).setTradingFee(BigNumber.from('2'), BigNumber.from('123'))).to.be.revertedWith('setTradingFee: market does not exist')
	})
	...
	it('fail user sets platform fee', async () => {
		...
		await expect(
			ideaTokenFactory.connect(userAccount).setPlatformFee(BigNumber.from('1'), BigNumber.from('123'))).to.be.revertedWith('Ownable: onlyOwner')
	})
	...
	it('fail set platform fee invalid market', async () => {
		...
		await expect(
			ideaTokenFactory.connect(adminAccount).setPlatformFee(BigNumber.from('2'), BigNumber.from('123'))).to.be.revertedWith('setPlatformFee: market does not exist')
	})
})