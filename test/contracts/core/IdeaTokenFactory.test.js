...
describe("core/IdeaTokenFactory", () => {
  ...
  beforeEach(async () => {
    ...
    await ideaTokenFactory
      ...
      .initialize(adminAccount.address, ideaTokenExchangeAddress)
  })
  ...
  it("can add market", async () => {
    ...
    expect(BigNumber.from("1").eq(await ideaTokenFactory.getNumMarkets())).to.be
      .equal
    expect(
      BigNumber.from("1").eq(
        await ideaTokenFactory.getMarketIDByName(marketName)
      )
    ).to.be.equal

    const marketDetailsByID = await ideaTokenFactory.getMarketDetailsByID(
      BigNumber.from("1")
    )
    const marketDetailsByName = await ideaTokenFactory.getMarketDetailsByName(
      marketName
    )
  ...
  it("can add token", async () => {
    ...
    const marketDetails = await ideaTokenFactory.getMarketDetailsByID(
      BigNumber.from("1")
    )
  ...
  it("fail add token with invalid name", async () => {
    ...
    await expect(
      ideaTokenFactory.addToken("some.invalid.name", BigNumber.from("1"))
  ...
  it("fail add token with same name twice", async () => {
    ...
    await ideaTokenFactory.addToken(tokenName, BigNumber.from("1"))
    await expect(
      ideaTokenFactory.addToken(tokenName, BigNumber.from("1"))
    ).to.be.revertedWith("addToken: name verification failed")
  })
  ...
  it("fail add token invalid market", async () => {
    ...
    await ideaTokenFactory.addToken(tokenName, BigNumber.from("1"))
    await expect(
      ideaTokenFactory.addToken(tokenName, BigNumber.from("2"))
    ).to.be.revertedWith("addToken: market does not exist")
  })
  ...
  it("can set trading fee", async () => {
    ...
    await ideaTokenFactory
      .connect(adminAccount)
      .setTradingFee(BigNumber.from("1"), BigNumber.from("123"))
    const marketDetails = await ideaTokenFactory.getMarketDetailsByID(
      BigNumber.from("1")
    )
  ...
  it("fail set trading fee invalid market", async () => {
    ...
    await expect(
      ideaTokenFactory
        .connect(adminAccount)
        .setTradingFee(BigNumber.from("2"), BigNumber.from("123"))
  ...
  it("can set platform fee", async () => {
    ...
    await ideaTokenFactory
      .connect(adminAccount)
      .setPlatformFee(BigNumber.from("1"), BigNumber.from("123"))
    const marketDetails = await ideaTokenFactory.getMarketDetailsByID(
      BigNumber.from("1")
    )
  ...
  it("fail set platform fee invalid market", async () => {
    ...
    await expect(
      ideaTokenFactory
        .connect(adminAccount)
        .setPlatformFee(BigNumber.from("2"), BigNumber.from("123"))
