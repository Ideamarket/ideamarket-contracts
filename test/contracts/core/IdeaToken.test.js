...
describe("core/IdeaToken", () => {
  ...
  it("admin can mint tokens", async () => {
    ...
    expect(tenPow18.eq(await ideaToken.balanceOf(userAccount.address))).to.be
      .true
  })
  ...
  it("admin can burn tokens", async () => {
    ...
    expect(
      BigNumber.from("0").eq(await ideaToken.balanceOf(userAccount.address))
    ).to.be.true
  })

  it("normal user cannot mint tokens", async () => {
    await expect(
      ideaToken.connect(userAccount).mint(userAccount.address, tenPow18)
  ...
  it("normal user cannot burn tokens", async () => {
    ...
    expect(tenPow18.eq(await ideaToken.balanceOf(userAccount.address))).to.be
      .true
    await expect(
      ideaToken.connect(userAccount).burn(userAccount.address, tenPow18)
    ).to.be.revertedWith("Ownable: onlyOwner")
  })
  ...
  it("user can transfer tokens", async () => {
    ...
    await ideaToken
      .connect(userAccount)
      .transfer(otherUserAccount.address, tenPow18)
    expect(
      BigNumber.from("0").eq(await ideaToken.balanceOf(userAccount.address))
    ).to.be.true
    expect(tenPow18.eq(await ideaToken.balanceOf(otherUserAccount.address))).to
      .be.true
  })
  ...
  it("user can approve other user", async () => {
    ...
    await ideaToken
      .connect(userAccount)
      .approve(otherUserAccount.address, tenPow18)
    expect(
      tenPow18.eq(
        await ideaToken.allowance(userAccount.address, otherUserAccount.address)
      )
    ).to.be.true
    await ideaToken
      .connect(otherUserAccount)
      .transferFrom(userAccount.address, otherUserAccount.address, tenPow18)
    expect(
      BigNumber.from("0").eq(await ideaToken.balanceOf(userAccount.address))
    ).to.be.true
    expect(tenPow18.eq(await ideaToken.balanceOf(otherUserAccount.address))).to
