...
contract('spells/SetTradingFeeSpell', async accounts => {
	...
	before(async () => {
		...
		spell = await SetTradingFeeSpell.new()
	})
	...
	it('can set trading fee', async () => {
		...
		const tradingFee = new BN((await factory.getMarketDetailsByID(new BN('1'))).tradingFeeRate)
