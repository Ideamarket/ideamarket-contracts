...
describe('evm/core/MultiAction', () => {
	...
	const baseCost = BigNumber.from('1000000000000000000') // 10**17 = $0.1
	const priceRise = 0
	const hatchTokens = 0
	...
	before(async () => {
		...
		TestcIMO = await ethers.getContractFactory('TestCDai')
		...
		MultiAction = await ethers.getContractFactory('MultiActionIMO')
	})
	...
	beforeEach(async () => {
		...
		cIMO = await TestcIMO.deploy(imo.address, comp.address, comptroller.address)
		await cIMO.deployed()
		await cIMO.setExchangeRate(tenPow18)
		...
		await interestManagerCompound
			...
			.initialize(ideaTokenExchange.address, imo.address, cIMO.address, comp.address, oneAddress)
	...
	it('can buy/sell tokens ETH', async () => {
		...
		const requiredInputForCost = (await router.getAmountsIn(buyCost, [weth.address, imo.address]))[0]
		...
		const outputFromSell = (await router.getAmountsOut(sellPrice, [imo.address, weth.address]))[1]
	...
	it('can buy/sell tokens SOME', async () => {
		const ideaTokenAmount = tenPow18.mul(BigNumber.from('25'))
		const buyCost = await ideaTokenExchange.getCostForBuyingTokens(ideaToken.address, ideaTokenAmount)
		const requiredInputForCost = (await router.getAmountsIn(buyCost, [someToken.address, imo.address]))[0]
		...
		const sellPrice = await ideaTokenExchange.getPriceForSellingTokens(ideaToken.address, tokenBalanceAfterBuy)
		const outputFromSell = (await router.getAmountsOut(sellPrice, [imo.address, someToken.address]))[1]
	...
	it('can buy/sell tokens 3-hop', async () => {
		...
		const requiredInputForCost = (
			await router.getAmountsIn(buyCost, [someOtherToken.address, weth.address, imo.address])
		)[0]
		...
		const outputFromSell = (
			await router.getAmountsOut(sellPrice, [imo.address, weth.address, someOtherToken.address])
		)[2]
	...
	it('can buy and fallback', async () => {
		...
		const requiredInputForFallbackCost = (
			await router.getAmountsIn(buyFallbackCost, [weth.address, imo.address])
		)[0]
		await multiAction.convertAndBuy(
			zeroAddress,
			ideaToken.address,
	...
	it('can buy and lock imo', async () => {
		const ideaTokenAmount = tenPow18.mul(BigNumber.from('25'))
		const buyCost = await ideaTokenExchange.getCostForBuyingTokens(ideaToken.address, ideaTokenAmount)
		await imo.mint(userAccount.address, buyCost)
		await imo.approve(multiAction.address, buyCost)
	...
	it('can buy and lock imo with fallback', async () => {
		const ideaTokenAmount = tenPow18.mul(BigNumber.from('25'))
		...
		await imo.mint(userAccount.address, buyFallbackCost)
		await imo.approve(multiAction.address, buyFallbackCost)
