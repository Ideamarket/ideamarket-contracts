...
// AVM-DONE
describe('avm/core/IdeaTokenExchange', () => {
	...
	before(async () => {
		...
		IdeaTokenExchange = await ethers.getContractFactory('IdeaTokenExchangeAVM')
	...
	async function reset() {
		...
		interestManagerStateTransferAVM = await InterestManagerStateTransferAVM.deploy()
		await interestManagerStateTransferAVM.deployed()
		...
		await interestManagerStateTransferAVM
		...
		await ideaTokenExchange
			...
			.initialize(
				...
				interestManagerStateTransferAVM.address,
				dai.address,
				oneAddress
			)
