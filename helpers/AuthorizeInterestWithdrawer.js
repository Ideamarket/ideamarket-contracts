...
async function run() {
  ...
  const idPair = await factoryContract.methods
    .getTokenIDPair(tokenAddress)
    .call()
  if (!idPair.exists) {
  ...
  const tokenName = (
    await factoryContract.methods.getTokenInfo(marketID, tokenID).call()
  ).name
  const marketName = (
    await factoryContract.methods.getMarketDetailsByID(marketID).call()
  ).name
  const executionTimestamp = shared.unixTimestampFromDateString(executionDate)
  const exchangeAddress = shared.loadDeployedAddress(
    network,
    "ideaTokenExchange"
  )
  ...
  const spellAddress = shared.loadDeployedAddress(
    network,
  ...
  const fax = web3.eth.abi.encodeFunctionCall(
    ...
    [exchangeAddress, tokenAddress, interestWithdrawerAddress]
  )
