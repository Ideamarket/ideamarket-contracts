...
interface IIdeaTokenExchange {
    ...
    function getPricesForSellingTokens(IIdeaTokenFactory.MarketDetails memory marketDetails, uint supply, uint amount) external pure returns (uint, uint, uint, uint);
    ...
    function getCostsForBuyingTokens(IIdeaTokenFactory.MarketDetails memory marketDetails, uint supply, uint amount) external pure returns (uint, uint, uint, uint);
